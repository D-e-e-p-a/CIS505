...
/**
 * @author Deepa class StudentIO has methods findAll(),insert(Student
 *         s),getString() and isNotNumber(String s)
 *
 */
public class StudentIO {
	...
	/**
	 * @return List<Student>
	 * @throws Exception this method scans the CSV file and store the data in Array
	 *                   list the array list is returned if the view button is
	 *                   pressed
	 */
	...
	/**
	 * @param s-student
	 * @throws Exception this method does two data storage jobs one to add the
	 *                   student object to the private Arraylist-studentList two it
	 *                   add the information to the CSV file and creates one if it
	 *                   doesn't exist.
	 */
	static void insert(Student s) throws Exception {
		...
		/*
		 * output.print(newLineTag); output.print(s.getFirstName());
		 * output.print(deLimiter); output.print(s.getLastName());
		 * output.print(deLimiter); output.print(s.getCourse());
		 * output.print(deLimiter); output.print(s.getGrade());
		 */
	 ...
	 * @param three
	 * @param four
	 * @return boolean this acts as a validation method for the input values
	 *         numbers, blank fields and special characters are rejected and not
	 *         saved the user is prompted with an error message.
	 * 
	 */
	public static boolean getString(String one, String two, String three, String four) {
		...
		// if no blank and no numbers and no special characters in the field then the
		// data is saved as student object
	...
	/**
	 * @param s
	 * @return boolean this method is used inside the getString(str,str,str,str), to
	 *         check the condition 2 if there is no number or special characters in
	 *         the input value this method returns true
	 * 
	 */
